...
| 2026-08-01 | **ORM = Drizzle, with an explicitly-approved migration path.** Full ORM chosen to give self-hosters a reliable update path; **Drizzle over Prisma** because it's TypeScript-native and tiny, supports SQLite *and* Postgres *and* Cloudflare D1, and so preserves the Node↔Workers portability Hono was chosen for (Prisma's query engine is heavy there). Generates readable `.sql` migrations an operator can review. **No auto-apply, ever — and no standing opt-in flag:** each upgrade needs explicit approval *at that time*, via CLI (`npm run migrate`) or an admin "upgrade required" screen the app boots into when migrations are pending. The app refuses normal operation against a mismatched schema. **A timestamped backup is written first** — trivial with single-file SQLite, so rollback is "restore that file." |
| 2026-08-01 | **Stack: Hono (backend) + Lit/Vite (frontend), TypeScript throughout.** **Hono over Fastify** — it runs on Node *and* Cloudflare Workers/Deno/Bun from one codebase, matching our two node profiles (self-hosted VPS + free-tier serverless) without a second path. **NestJS rejected** — DI/decorators/modules are heavy abstraction for a ~10-endpoint app and a barrier to casual contributors, against the lightweight principle. Validation wires `ajv` directly against `spec/schema/node-v1.schema.json`, keeping one source of truth. Vite is dev-server/bundler only; shipped output is plain Web Components. |
