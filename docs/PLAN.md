...
| 2026-08-01 | **ORM = Prisma** (supersedes an earlier Drizzle call). Full ORM for a reliable self-hoster update path. **Why Prisma over Drizzle:** one `schema.prisma` serves **both SQLite and Postgres**, honoring our "SQLite default, Postgres optional" promise from a single codebase — Drizzle's schemas are dialect-specific (`sqlite-core` vs `pg-core`) and would mean maintaining two definitions. Also: mature guided migrations (`migrate dev` / `migrate deploy` / `migrate status`), better docs for contributors, and **Prisma 7 is Rust-free**, removing the platform-specific engine binary that made self-hosting awkward. Both tools emit reviewable `.sql` — that was not a differentiator. **Accepted cost:** Cloudflare Workers/D1 is a poor fit (bundle size vs. the 1 MB free tier), so the $0 serverless tier shifts to Node-runtime hosts (Vercel/Netlify/Fly/Railway) + Neon rather than Workers/D1. **Migrations never auto-apply and there is no standing opt-in:** each upgrade is approved *at that time* via CLI or an admin "upgrade required" screen the app boots into when `migrate status` reports pending work; a timestamped backup is written first. |
| 2026-08-01 | **Stack: Hono (backend) + Lit/esbuild (frontend), TypeScript throughout.** **Hono over Fastify** — small, fast, and runtime-portable (Node, Deno, Bun, edge), keeping the self-hosted and serverless profiles on one codebase. *(Note: with Prisma chosen, the Cloudflare Workers path specifically is deprioritized — see the ORM entry — but Hono remains a good fit and keeps options open.)* **NestJS rejected** — DI/decorators/modules are heavy abstraction for a ~10-endpoint app and a barrier to casual contributors, against the lightweight principle. Validation wires `ajv` directly against `spec/schema/node-v1.schema.json`, keeping one source of truth. **esbuild only — no Vite:** one small dependency, no plugin ecosystem or framework opinions. `--bundle --watch` for builds, `--servedir` for the dev loop; no HMR (a plain reload is fine for Lit). Shipped output is plain Web Components. |
