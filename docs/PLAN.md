...
| 2026-08-01 | **Stack: Hono (backend) + Lit/Vite (frontend), TypeScript throughout.** **Hono over Fastify** — small, fast, and runtime-portable (Node, Deno, Bun, edge), keeping the self-hosted and serverless profiles on one codebase. *(Note: with Prisma chosen, the Cloudflare Workers path specifically is deprioritized — see the ORM entry — but Hono remains a good fit and keeps options open.)* **NestJS rejected** — DI/decorators/modules are heavy abstraction for a ~10-endpoint app and a barrier to casual contributors, against the lightweight principle. Validation wires `ajv` directly against `spec/schema/node-v1.schema.json`, keeping one source of truth. Vite is dev-server/bundler only; shipped output is plain Web Components. |
